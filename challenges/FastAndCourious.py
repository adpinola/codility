...
def solution(A):
    ...
    total = 0
    for k in range(N-1):
        partials[k] = A[k + 1] - A[k]
        if partials[k] >= longestDistance:
            longestDistance = partials[k]
            cityIndex = k
        total += partials[k] * (k + 1)
    print("total",total)
    for i in range(cityIndex, N-1):
        total -= partials[i] * (cityIndex + 1)
        print("total",total)
